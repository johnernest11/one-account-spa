...
import GoogleUserDetailsForm from '@/components/users-management-page/GoogleUserDetailsForm.vue'
import { ref } from 'vue'

const props = defineProps<{ user: GoogleUserResponse; roleFilter: number | string | null }>()
...
<template>
  ...
  >
    ...
    <p class="mt-16 text-center font-menu font-bold text-primary-600 dark:text-primary-400">
      {{ props.user?.name?.fullName }}
    </p>
    <!-- End Name -->
    <!-- Start Email -->
    <p class="text-center text-sm text-surface-700 dark:text-surface-400">{{ props.user?.primaryEmail }}</p>
    <!-- End Email -->
    <!-- Start Role Tags -->
    ...
    >
      <GoogleUserDetailsForm
        ...
        @user-updated="toggleUserDetailsDialog" 
      />
    </Dialog>
    <!-- End Update-User Dialog -->
    <!-- Start Deactivated Flag -->
    <footer
      v-if="props.user.suspended"
